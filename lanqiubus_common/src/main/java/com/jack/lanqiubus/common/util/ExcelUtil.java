...
public class ExcelUtil {
     ...
     */
    public static boolean createHSSFWorkBook(ExcelBean excelBean) {
        ...
        return writeXSLXFile(workbook, excelBean);
    }
     ...
     * @param excelBean 文件存放的目录
     */
    public static boolean createHSSFWorkBookForMerge(ExcelBean excelBean) {
        Workbook workbook = new HSSFWorkbook();
        Sheet sheet = workbook.createSheet(excelBean.getSheetName());
        ...
        for (Map.Entry entry : dataMap.entrySet()) {
            ...
            if (mergeColName != null && !"".equals(mergeColName.trim()) && !"null".equals(mergeColName)) {
                sheet.addMergedRegion(new CellRangeAddress(firstRow, (firstRow + data.size() - 1), titleIndex, titleIndex));    // 合并第一列的第一行到第4行，
                firstRow = rowNum;
            }
        }
     ...
     *
     * @param workbook 工作簿
     * @param bean     excel文件存放路径
     */
    private static boolean writeXSLXFile(Workbook workbook, ExcelBean bean) {
     ...
     * @param template
     */
    public static void readExcel(File template) {
        try {
            Workbook wb = WorkbookFactory.create(template);
            int sheetCnt = wb.getNumberOfSheets();          // 获取有多少sheet
            for (int i = 0; i < sheetCnt; i++) {
                Sheet sheet = wb.getSheetAt(i);
                int rowNum = sheet.getPhysicalNumberOfRows();     // 获取每个sheet中有多少行
                for (int j = 1; j <= 100; j++) {
                    Row row = sheet.createRow(j);
                    if (row == null) continue;
                    Map<String, Object> map = new HashMap<String, Object>();
//                    map.put("uid", processCellTypeConvert(row.getCell(0), true));     // 获取第一个单元格的数据(uid)
//                    map.put("siteId", processCellTypeConvert(row.getCell(1)));          // 获取第二个单元格的数据(siteId)
//                    map.put("domain", processCellTypeConvert(row.getCell(2)));          // 获取第三个单元格的数据(domani)
//                    map.put("platform", processCellTypeConvert(row.getCell(3)));     // 获取第四个单元格的数据(platform)
//                    accountMapList.add(map);
                    Cell cell = row.createCell(0);
                    cell.setCellValue(j);
                }
            }

            FileOutputStream fileOut = new FileOutputStream("H:\\haoxiaolei\\Desktop\\result.xlsx");
            wb.write(fileOut);
            fileOut.close();
        } catch (IOException e) {
            e.printStackTrace();
     ...
     * @return Workbook的模板
     */
    public static Workbook readExcelTemplate(File template) {
        Workbook wb = null;
        try {
     ...
     */
    public static boolean fillDataIntoWorkbook(Workbook workbook, ExcelBean bean) {
...
//                    sheet.autoSizeColumn(cellNum);
     ...
     * @Copyright: 2013 ihome.com
     */
    public static class ExcelBean implements Serializable {
